...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // Stocker un "token" simple dans localStorage
        localStorage.setItem("token", data.token || pseudo); 
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded shadow-md w-full max-w-sm">
        ...
        <div className="mb-4">
          ...
          <input type="text" className="input input-bordered w-full" value={pseudo} onChange={(e) => setPseudo(e.target.value)} />
        </div>
        <div className="mb-6">
        ...
        <button type="submit" className="btnOpenSidebar w-full">Se connecter</button>
      </form>
    </div>
  );
}
